...
public class DongDataAnalysis extends JFrame {
    //법정동과 이용금액을 저장
    ...
    public DongDataAnalysis() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\Users\\sunni\\OneDrive\\바탕 화면\\창의프로젝트\\유성구_법정동별 소비금액 데이터_2020.csv"))) {
            ...
            for (Map.Entry<String, Double> entry : dongAmountMap.entrySet()) {
                ...
                // 데이터셋에 값 추가
                dataset.setValue(amount, "2020년 법정동별 이용 금액", dongName);
            }
            ...
            System.out.println("가장 많이 소비한 동은 " + maxDongName + "에 " + formattedMaxAmount + "원 입니다.");
            ...
            System.out.println("가장 적게 소비한 동은 " + minDongName + "에 " + formattedMinAmount + "원 입니다.");

            // 차트 생성
            JFreeChart chart = ChartFactory.createBarChart(
                    "2020년 법정동별 이용 금액", // 차트 제목
                    "법정동명", // X축 레이블
                    "이용 금액", // Y축 레이블
                    dataset // 데이터셋
            );

            // X축 데이터의 라벨을 법정동명으로 설정
