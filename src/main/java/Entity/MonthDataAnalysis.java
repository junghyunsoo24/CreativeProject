...
public class MonthDataAnalysis extends JFrame {
    //기준월별과 이용금액을 저장
    ...
    public MonthDataAnalysis() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\Users\\sunni\\OneDrive\\바탕 화면\\창의프로젝트\\유성구_법정동별 소비금액 데이터_2020.csv"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (dongAmountMap.containsKey(month)) {
                    double currentAmount = dongAmountMap.get(month);
                    dongAmountMap.put(month, currentAmount + amount);
                } else {
                    // 기준월이 처음 나온 경우, 새로운 항목으로 추가
                    dongAmountMap.put(month, amount);
                }
            ...
            for (Map.Entry<String, Double> entry : dongAmountMap.entrySet()) {
                ...
                // 데이터셋에 값 추가
                dataset.setValue(amount, "2020년 기준월별 이용 금액", month);
            }
            ...
            System.out.println("가장 많이 소비한 기준월은 " + maxMonth + "월에 " + formattedMaxAmount + "원 입니다.");
            ...
            System.out.println("가장 적게 소비한 기준월은 " + minMonth + "월에 " + formattedMinAmount + "원 입니다.");

            // 차트 생성
            JFreeChart chart = ChartFactory.createBarChart(
                    "2020년 기준월별 이용 금액", // 차트 제목
                    "기준월", // X축 레이블
                    "이용 금액", // Y축 레이블
                    dataset // 데이터셋
            );

            // X축 데이터의 라벨을 기준월명으로 설정
