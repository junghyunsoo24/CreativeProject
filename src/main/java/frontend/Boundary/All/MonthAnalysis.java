...
import java.util.TreeMap;
    public class MonthAnalysis extends Application {
        ...
        public void start(Stage primaryStage) throws IOException, ClassNotFoundException {
            ...
            Label DongCheckLabel = new Label("선택한 대분류는 " + sectors + "에 "  + formattedSum + "원 입니다.");
            ...
            Label maxLabel = new Label("가장 많이 소비한 대분류는 " + sectors + "에 " + formattedMaxAmount + "원 입니다.");
            ...
            Label minLabel = new Label("가장 적게 소비한 대분류는 " + sectors + "에 " + formattedMinAmount + "원 입니다.");

            Long sums = ClientApp.getDB().selectRequest(ProtocolQuery.selectSum, ProtocolType.CA, sectors.getCode());

            // 데이터 생성
            series.getData().add(new XYChart.Data<>(maxDongName, maxAmount));
            series.getData().add(new XYChart.Data<>(minDongName, minAmount));
            series.getData().add(new XYChart.Data<>(sectors.name(),sum));
